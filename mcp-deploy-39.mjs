...
const payload = JSON.parse(readFileSync('/tmp/deploy-39.json', 'utf8'));
const token = await getOidcToken();
const transport = new StreamableHTTPClientTransport(new URL('https://mcp.vercel.com'), { requestInit: { headers: { Authorization: `Bearer ${token}`, Accept: 'application/json, text/event-stream' } } });
